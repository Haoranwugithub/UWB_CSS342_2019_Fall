...
void testCopyConstructor() {
	...
	if (arr2.maxLength() != 10) {
			printf("testCopyConstructor FAILED, capacity doesn't match. expect: %d, actual: %d\n",
					arr.maxLength(), arr2.maxLength());
			return;
	}
	
	if (arr2.count() != 5) {
			printf("testCopyConstructor FAILED, count doesn't match. expect: %d, actual: %d\n",
    ...
    for (int i = 0; i<arr.count(); i++) {
		if (arr2[i] != arr[i]) {
			printf("testCopyConstructor FAILED, expect: %d, actual: %d, i=%d\n",
					arr[i], arr2[i], i);
			return;
		}
	}

	printf("testCopyConstructor PASSED\n");
}
